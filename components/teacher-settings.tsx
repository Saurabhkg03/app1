...
export function TeacherSettings({ onBack }: { onBack: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      // 1. Update Firebase Auth profile for display in the app header
      await updateProfile(authUser, { displayName: trimmedName })

      // 2. Update the teacher's main profile document in Firestore
  ...
  return (
    <div className="space-y-6">
      ...
      <h2 className="text-2xl font-bold text-gray-800 flex items-center">
        <User className="w-6 h-6 mr-2 text-primary" /> Teacher Settings
      </h2>
      ...
      <div className="p-6 bg-white rounded-xl shadow-md border border-gray-100 space-y-4">
        <div>
          ...
          <input
            ...
            placeholder="Enter the name your students will see"
          />
          <p className="text-xs text-gray-500 mt-2">
            This name will be displayed to your students.
          </p>
        </div>
